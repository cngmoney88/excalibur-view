...
Square takes the payment; the license is signed here, beside the key.
`tools/square.py setup` makes the two payment links the website's Buy buttons
point at (Office per user, and a year of updates), each asking the buyer for
the company name and the administrator's email. `tools/square.py issue` turns
each paid order into a signed `.evlicense` and the email to send with it —
working out for itself whether it is a first purchase, more seats on a license
that exists, or a renewal. Nothing is charged automatically, nothing is issued
twice, and nothing about a customer is anywhere but that PC.
`tools/square_check.py` runs the lot against a stand-in Square with a throwaway
key. See `docs/checkout.md`.

## Claude, in the office and at the screen

**Help → Connect Claude** sets it up on a seat: the server makes a key that can
do nothing but ask, and Hyperview adds itself to Claude Desktop's connectors
(`Hyperview.exe --mcp`). With no office server — one person on the free app —
it connects the window alone: what is open, reading and looking at sheets,
`takeoff_totals` (the markups list's own totals for the drawing open), and
proposals with a Fix button. Claude is offered only those tools and the
ready-made requests that work without a server. Quit Claude Desktop **from the tray** — closing its
window is not enough — and open it again. **Help → Test Claude Connection**
checks everything that decides whether it works and says what to fix in words;
`Hyperview.exe --mcp-test` does the same from a command line and leaves the
answer in `claude-check.txt` beside the program. Every time Claude starts the
connector it is written down in `claude-bridge.log`, so "it didn't seem to
work" has an answer.

There is no API key and nothing is billed per question: Claude Desktop is the
person's own Claude plan. Everyone in the office uses the **same** office
server through it — one set of jobs, one takeoff — each signed in to Claude
with their own account (a Claude Team plan is the way to give a whole office
seats; one personal login shared between people is against Claude's terms).

What Claude can do:

- **Ask the office** — the jobs, the drawing sets (which issue is current,
  which was superseded, what takeoff was carried forward and still needs a
  look), the quantities, the cut list, what a revision cost, what was counted
  twice, which sheets have no scale. The server speaks MCP at `/mcp`.
- **Work the window** on the person's own computer — what they have open,
  open a set from the office or a PDF, turn to a sheet by number, **read** a
  sheet's words line by line with where each line is, **look** at a sheet or
  part of one as a picture, list its markups, point at something (select and
  zoom), and run the office's plugin checks.
- **Propose** — lengths, counts, areas, clouds and notes; a sheet's scale read
  from its title block; a markup's subject, quantity or slope. Proposals appear
  in the window as a list, each outlined on the sheet with a **Fix** button.
  Nothing is drawn or changed until the person clicks, and each click is one
  step on the Undo list.
- **Ready-made requests** in Claude Desktop's menu: check the takeoff on this
  sheet, find each sheet's scale, draft RFIs for this sheet, a job's
  quantities, what a reissue changed, a job's cut list.

It never draws on its own, **on purpose**. The office server has no tool that
draws a markup, changes a scale or edits a takeoff, and `crates/server/src/mcp.rs`
has a test that fails if one ever appears; the window's tools have the same
test in `crates/hyperview/src/desk.rs`. A program whose whole argument is that
every quantity traces back to something a person clicked must not grow a door
a language model can click through — so Claude proposes, and a person clicks.

The answers carry their own warnings, in words rather than as a field somebody
might not read — a takeoff short three sheets says so in the text, because the
thing reading it is going to write sentences somebody prices a job off.

`hyperview://open?set=<id>&page=<n>` (or `?project=<id>`) opens a set at a
sheet in the window — from FabWire, an email, or Claude — starting Hyperview if
it is not open. The installer registers it for the person.
